		...
		try {
			...
			if (month == null) {
				...
				try {
					...
					isError = true;
					errorMessage = errorMessage + "Invalid Month Input. \n";
				}
			...
			if (day == null) {
				...
				try {
					...
					isError = true;
					errorMessage = errorMessage + "Invalid Day Input. \n";
				}
			}
			...
			if (year == null) {
				...
				try {
					...
					isError = true;
					errorMessage = errorMessage + "Invalid Year Input. \n";
				}
			...
			if ((hike.equals("Gardiner Lake") == false) && (hike.equals("The Hellroaring Plateau")) == false && 
					(hike.equals("The Beaten Path")) == false) {
				isError = true;
				errorMessage = errorMessage + "Invalid Hike Input. \n";
			}
			else {
			...
			if (length == null) {
				...
				try {
					...
					isError = true;
					errorMessage = errorMessage + "Invalid Length Input. \n";
				}
				
			}
			
			// Get # of hikers
			hikers = request.getParameter("hikers");
			try {
				hikersInt = Integer.parseInt(hikers);
			}
			catch (NumberFormatException e) {
				isError = true;
				errorMessage = errorMessage + "Invalid Hiker Input. \n";
			}
			...
			price = dayPrice * lengthInt * hikersInt;
			...
			if(isError == false) {
				out.println("<!DOCTYPE html>\n" + 
						...
						"\n" + 
						"<body>\n" + 
						"	<form name=hikeform action=\"http://localhost:8080/dea_s_hw8/hikeServlet\" onsubmit=\"return validateForm()\" method=GET" + 
						"	\n" + 
						...
						"				Hikers: <input type=\"TEXT\" name=\"hikers\" value=" + hikers + 
						"			</fieldset>\n" + 
						"		</div>" +
						"		\n" + 
						"	</div>\n");
